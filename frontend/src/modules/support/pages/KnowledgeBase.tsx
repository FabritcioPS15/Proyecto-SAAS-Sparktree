...
import { Plus, Search, Edit, Trash2, List, LayoutGrid, BookOpen } from 'lucide-react';
    ...
    <PageContainer>
      ...
      >
        <form onSubmit={(e) => { e.preventDefault(); addNotification({ type: 'success', title: 'Artículo creado', message: `Se ha creado el artículo ${formTitle}` }); setShowCreateModal(false); }} className="space-y-4">
          ...
          <button type="submit" className="w-full py-3.5 bg-gradient-to-r from-accent-500 to-emerald-500 text-black text-[10px] font-black uppercase tracking-widest rounded-xl hover:from-accent-600 hover:to-emerald-600 transition-all shadow-md">Crear Artículo</button>
        </form>
      </Modal>
    </PageContainer>
  );
