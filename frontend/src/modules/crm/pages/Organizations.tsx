...
export const Organizations = () => {
  ...
  const getPlanDescription = (plan: string) => {
    const descriptions: { [key: string]: { title: string; desc: string; features: string[]; limit: string } } = {
      'free': {
        title: 'Starter',
        desc: 'Perfecto para pequeÃ±as empresas que comienzan',
        features: ['Hasta 5 usuarios', '1,000 mensajes/mes', 'Soporte bÃ¡sico', '1 organizaciÃ³n'],
        limit: 'Ideal para empezar'
      },
      'pro': {
        title: 'Growth',
        desc: 'Para empresas en crecimiento con mayores necesidades',
        features: ['Hasta 25 usuarios', '10,000 mensajes/mes', 'Soporte prioritario', '5 organizaciones', 'API bÃ¡sica'],
        limit: 'Popular para equipos'
      },
      'enterprise': {
        title: 'Global',
        desc: 'SoluciÃ³n completa para grandes corporaciones',
        features: ['Usuarios ilimitados', 'Mensajes ilimitados', 'Soporte 24/7', 'Organizaciones ilimitadas', 'API completa', 'SLA garantizado'],
        limit: 'MÃ¡xima capacidad'
      }
    };
  ...
  const handleUpdateOrg = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error al actualizar la organizaciÃ³n');
    }
  };

  const handleDeleteOrg = async (id: string) => {
    if (!window.confirm('Â¿Eliminar esta organizaciÃ³n?')) return;
    try {
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {(Array.isArray(orgs) ? orgs : []).map((org) => (
              <div key={org.id} className="group bg-white dark:bg-dark-card p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800/50 shadow-sm hover:shadow-xl transition-all duration-500 relative overflow-hidden flex flex-col">
                ...
                <div className="flex items-start justify-between mb-6 relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 dark:bg-white/5 rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-accent-500 transition-all duration-500 group-hover:scale-110 shadow-inner">
          ...
          <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-slate-800/50 overflow-hidden">
                  ...
                  {orgs.map((org) => (
                    <tr key={org.id} className="hover:bg-slate-50 dark:hover:bg-white/2 transition-all">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 dark:bg-white/5 rounded-lg flex items-center justify-center">
