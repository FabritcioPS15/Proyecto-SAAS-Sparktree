...
import { Search, ChevronLeft, ChevronRight, ArrowUpDown, User, Trash2, Phone, UserCheck, CheckSquare, Square, MessageSquare, List, LayoutGrid, Cpu, Zap, Filter } from 'lucide-react';
...
export const Clients = () => {
  ...
  const handleDelete = async (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (window.confirm('Â¿Eliminar cliente y sus conversaciones?')) {
      try {
        await deleteUser(id);
  ...
  const handleBulkDelete = async () => {
    if (window.confirm(`Â¿Eliminar los ${selectedIds.size} clientes seleccionados y sus chats?`)) {
      try {
        ...
        alert('Error al eliminar masivamente. Algunos clientes podrÃ­an no haberse borrado.');
      }
    }
  };
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Directorio de"
        highlight="Clientes"
        description="GestiÃ³n integral de interacciones."
        icon={User}
        action={
      ...
      <PageBody>
        {/* Filters Area */}
        <div className="bg-white dark:bg-dark-card rounded-xl p-4 border border-gray-200 dark:border-white/5 shadow-sm mb-4">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-emerald-500 transition-colors" />
              <input
                type="text"
                placeholder="Buscar por nÃºmero o agente..."
                value={searchTerm}
                onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }}
                className="w-full pl-10 pr-4 py-2.5 dark:bg-dark-card/50 border border-gray-200 dark:border-white/5/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-gray-900 dark:text-white text-sm"
              />
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <select
                ...
                className="px-4 py-2.5 dark:bg-dark-card/50 border border-gray-200 dark:border-white/5/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-gray-900 dark:text-white text-sm cursor-pointer min-w-[160px]"
              >
              ...
              <div className="flex items-center dark:bg-dark-card rounded-lg p-1 border border-gray-200 dark:border-white/5/50">
                <button
                  onClick={() => setViewMode('table')}
                  className={`p-1.5 rounded-md transition-all ${viewMode === 'table' ? 'bg-white dark:bg-dark-card shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
                ...
                <button
                  ...
                  className={`p-1.5 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-gray-700 shadow-sm text-emerald-600 dark:text-emerald-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
        ...
        <div>
          {viewMode === 'table' ? (
            <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-slate-800/50 overflow-hidden shadow-sm">
              <div className="overflow-x-auto text-[13px]">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50/50 dark:bg-dark-card border-b border-slate-100 dark:border-slate-800">
                      ...
                      <th className="px-5 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px]">Canales</th>
                      <th className="px-5 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px]">LÃ­nea</th>
                      <th className="px-5 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px] cursor-pointer" onClick={() => handleSort('lastInteraction')}>
                        <div className="flex items-center gap-2">Ãšltima Vez <ArrowUpDown className="w-3 h-3" /></div>
                      </th>
                      <th className="px-5 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px] text-right">AcciÃ³n</th>
                    </tr>
                  </thead>
              ...
              <div className="px-5 py-3 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between/30 dark:bg-transparent">
                ...
                <div key={user.id} className={`group bg-white dark:bg-dark-card p-5 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden flex flex-col ${selectedIds.has(user.id) ? 'ring-2 ring-accent-500' : ''}`}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs mt-1">
                          ...
                          <span>{user.serviceNumber || 'Sin lÃ­nea'}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex gap-1">
                  ...
                  <div className="space-y-3 flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">ConexiÃ³n</span>
                      <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ãšltima vez</span>
