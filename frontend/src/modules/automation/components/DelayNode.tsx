...
export const DelayNode = ({ data }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-950 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 w-48 overflow-hidden transition-all hover:shadow-slate-500/10 group node-container">
      <Handle type="target" position={Position.Top} className="!bg-white dark:!bg-slate-700 !border-slate-400 group-hover:!bg-slate-500" />
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-white dark:!bg-slate-700 !border-slate-400 group-hover:!bg-slate-500" />
    </div>
  );
};