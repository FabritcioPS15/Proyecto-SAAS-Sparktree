...
export const DelayNode = ({ data }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-black rounded-sm shadow-xl border-2 border-black dark:border-white/10 w-48 overflow-hidden transition-all hover:scale-[1.02] group">
      <Handle type="target" position={Position.Top} className="w-3 h-3 bg-black border-2 border-white dark:border-dark-card" />
      ...
      <div className="p-6 text-center bg-white dark:bg-dark-card">
      ...
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 bg-black border-2 border-white dark:border-dark-card" />
    </div>
  );
};
