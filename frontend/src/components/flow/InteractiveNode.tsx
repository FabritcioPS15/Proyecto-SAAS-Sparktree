...
export const InteractiveNode = ({ data }: any) => {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border-2 border-primary-500 w-72 overflow-hidden transition-all hover:shadow-primary-500/10">
      <Handle type="target" position={Position.Top} className="w-3 h-3 bg-primary-500 border-2 border-white dark:border-gray-800" />
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-2 pt-2">
          {data.buttons?.map((btn: any, i: number) => (
            <div 
              key={btn.id || i} 
              className="px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-black text-primary-600 dark:text-primary-400 text-center relative hover:border-primary-500 transition-colors uppercase tracking-widest"
            >
              {btn.title}
              <Handle 
                 type="source" 
                 ...
                 id={btn.id} 
                 className="w-3 h-3 bg-primary-500 right-[-6px] top-1/2 transform -translate-y-1/2 border-2 border-white dark:border-gray-800" 
              />
            </div>
          ))}
