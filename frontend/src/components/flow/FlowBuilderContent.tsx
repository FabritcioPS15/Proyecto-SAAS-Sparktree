...
export const FlowBuilderContent = ({ flowData, onBack }: FlowBuilderContentProps) => {
  ...
  const updateFlowTriggers = (newTriggers: string[]) => {
    ...
    setNodes((nds) => nds.map((n) => n.type === 'trigger' ? { ...n, data: { ...n.data, keywords: newTriggers } } : n));
  };
  ...
  const chatNodes = [
    ...
    { id: 'interactive', icon: Grid3X3, label: 'Botones/Listas', sub: 'Interacción', color: 'purple' },
    { id: 'media', icon: Play, label: 'Multimedia', sub: 'Imágenes y Vídeo', color: 'pink' },
  ...
  const integrationNodes = [
    ...
    { id: 'handoff', icon: UserCheck, label: 'Agente Humano', sub: 'Pasar a asesor', color: 'rose' },
  ];
  ...
  const renderNodeButton = (node: any) => (
    <button
      ...
      className="group flex items-center gap-3 bg-white dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 px-3 py-2.5 rounded-xl hover:border-indigo-500/40 hover:shadow-lg hover:shadow-indigo-500/5 transition-all text-left w-full"
    >
    ...
    <div className="absolute inset-0 flex flex-col bg-slate-50 dark:bg-[#0f1117] overflow-hidden">
      ...
      <div className="px-8 py-5 bg-white dark:bg-[#11141b] border-b border-gray-100 dark:border-gray-800 flex items-center justify-between shadow-sm z-20 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          <button onClick={clearCanvas} className="px-5 py-2.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-gray-100 dark:border-gray-700 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-rose-50 hover:text-rose-600 hover:border-rose-100 transition-all flex items-center gap-2">
            <Trash2 className="w-3.5 h-3.5" /><span>Limpiar</span>
          </button>
          <button onClick={() => setShowSimulator(true)} className="px-5 py-2.5 bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/50 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-indigo-50 transition-all flex items-center gap-2">
          ...
          <div ref={reactFlowWrapper} className="w-full h-full">
            ...
            >
              ...
              <div className="absolute top-6 left-6 z-10 flex gap-2">
                ...
                <div className="flex bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-1.5 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-xl">
                  <button onClick={() => setShowMinimap(!showMinimap)} className={`p-2 rounded-xl transition-all ${showMinimap ? 'text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10' : 'text-gray-400'}`}><Layers className="w-4 h-4" /></button>
                  <button onClick={() => setShowGrid(!showGrid)} className={`p-2 rounded-xl transition-all ${showGrid ? 'text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10' : 'text-gray-400'}`}><Grid3X3 className="w-4 h-4" /></button>
                </div>
              </div>
          ...
          <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
            <div className="flex bg-slate-100/80 dark:bg-slate-800/50 p-1 rounded-xl border border-gray-100 dark:border-gray-700/50">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'nodos' ? 'bg-white dark:bg-gray-700 text-indigo-600 dark:text-indigo-400 shadow-md border border-indigo-100 dark:border-indigo-500/20' : 'text-gray-400 hover:text-gray-600'}`}
              >
                <Plus className="w-3 h-3" /> Nodos
              </button>
              <button
                onClick={() => setActiveTab('propiedades')}
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'propiedades' ? 'bg-white dark:bg-gray-700 text-indigo-600 dark:text-indigo-400 shadow-md border border-indigo-100 dark:border-indigo-500/20' : 'text-gray-400 hover:text-gray-600'}`}
              >
          ...
          {activeTab === 'propiedades' && selectedNode && (
            <div className="flex-1 overflow-y-auto px-3 py-3">
              ...
              {selectedNode.type === 'trigger' && (
                <div className="p-5 bg-emerald-50 dark:bg-emerald-500/5 rounded-3xl border border-emerald-100 dark:border-emerald-500/20">
                  ...
                  <textarea
                    ...
                    onChange={(e) => updateFlowTriggers(e.target.value.split(',').map(k => k.trim()).filter(k => k.length >= 2))}
                    onBlur={pushToHistory}
                    className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-emerald-200 dark:border-emerald-800 rounded-2xl text-sm outline-none resize-none h-28"
                    placeholder="hola, ayuda, info..."
                  />
                  <p className="text-[9px] text-emerald-600/70 mt-2 font-bold italic">Separa por comas (mín. 2 chars)</p>
                </div>
              )}
              ...
              {selectedNode.type === 'media' && (
                <div className="p-5 bg-pink-50 dark:bg-pink-500/5 rounded-3xl border border-pink-100 dark:border-pink-500/20 space-y-4">
                  <div>
                    <label className="block text-[10px] font-black text-pink-700 dark:text-pink-400 uppercase mb-2">URL del Archivo</label>
                    <input type="text" value={selectedNode.data.mediaUrl || ''} onChange={(e) => updateNodeData({ mediaUrl: e.target.value })} onBlur={pushToHistory} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-pink-200 rounded-2xl font-mono text-[10px]" placeholder="https://..." />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[9px] font-black text-pink-700 uppercase mb-2">Tipo</label>
                      <select value={selectedNode.data.mediaType || 'image'} onChange={(e) => { updateNodeData({ mediaType: e.target.value }); pushToHistory(); }} className="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-pink-200 rounded-xl text-[10px] font-black uppercase">
                    ...
                    <div className="flex items-center gap-2 pt-5">
                      <input type="checkbox" checked={!!selectedNode.data.isViewOnce} onChange={(e) => { updateNodeData({ isViewOnce: e.target.checked }); pushToHistory(); }} className="w-4 h-4 accent-pink-500" />
                      <label className="text-[9px] font-black text-pink-700 uppercase">1 Vista</label>
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-pink-700 dark:text-pink-400 uppercase mb-2">Pie de foto (Caption)</label>
                    <textarea value={selectedNode.data.caption || ''} onChange={(e) => updateNodeData({ caption: e.target.value })} onBlur={pushToHistory} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-pink-200 rounded-2xl text-[10px] resize-none" rows={2} placeholder="Escribe algo..." />
                  </div>
                </div>
              )}
              ...
              {selectedNode.type === 'interactive' && (
                <div className="p-5 bg-purple-50 dark:bg-purple-500/5 rounded-3xl border border-purple-100 dark:border-purple-500/20 space-y-3">
                  <label className="block text-[10px] font-black text-purple-700 dark:text-purple-400 uppercase tracking-widest">Mensaje Interactivo</label>
                  <textarea value={selectedNode.data.bodyText || ''} onChange={(e) => updateNodeData({ bodyText: e.target.value })} onBlur={pushToHistory} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-purple-200 rounded-2xl text-sm resize-none" rows={3} placeholder="Describe las opciones..." />
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-[10px] font-black text-purple-600 uppercase">Botones (Máx 3)</label>
                    ...
                    {(selectedNode.data.buttons || []).map((btn: any, idx: number) => (
                      <div key={idx} className="flex gap-2 mb-2 group/btn">
                        <input type="text" value={btn.title} onChange={(e) => { const nb = [...(selectedNode.data.buttons || [])]; nb[idx] = { ...nb[idx], title: e.target.value }; updateNodeData({ buttons: nb }); }} onBlur={pushToHistory} className="flex-1 px-3 py-2 bg-white dark:bg-gray-800 border border-purple-200 rounded-xl text-[11px] font-bold" />
                        <button onClick={() => { updateNodeData({ buttons: selectedNode.data.buttons.filter((_: any, i: number) => i !== idx) }); pushToHistory(); }} className="p-2 opacity-0 group-hover/btn:opacity-100 text-rose-500 hover:bg-rose-50 rounded-lg transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    ))}
                    {(!selectedNode.data.buttons || selectedNode.data.buttons.length < 3) && (
                      <button onClick={() => updateNodeData({ buttons: [...(selectedNode.data.buttons || []), { id: `btn-${Date.now()}`, title: 'Nuevo Botón' }] })} className="w-full py-3 border-2 border-dashed border-purple-200 dark:border-purple-800 rounded-2xl text-[9px] font-black text-purple-600 uppercase hover:bg-purple-50 transition-all">
                        + Añadir Botón
                      </button>
                    )}
                  </div>
              ...
              {selectedNode.type === 'handoff' && (
                <div className="p-5 bg-rose-50 dark:bg-rose-500/5 rounded-3xl border border-rose-100 dark:border-rose-500/20 text-center">
                  <div className="w-14 h-14 bg-rose-100 dark:bg-rose-900/40 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  ...
                  <div className="text-left">
                    <label className="block text-[9px] font-black text-rose-700 uppercase mb-2">Mensaje de Despedida</label>
                    <textarea value={selectedNode.data.message || ''} onChange={(e) => updateNodeData({ message: e.target.value })} onBlur={pushToHistory} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-rose-200 rounded-2xl text-[10px] resize-none" rows={2} placeholder="Un momento, te paso con un asesor..." />
                  </div>
                </div>
              )}
