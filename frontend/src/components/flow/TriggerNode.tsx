...
export const TriggerNode = ({ data }: any) => {
  return (
    <div className="bg-white dark:bg-black rounded-sm shadow-xl border-2 border-black dark:border-white/10 w-64 transition-all hover:scale-[1.02]">
      <div className="bg-black px-4 py-3 flex items-center justify-between border-b border-white/5 rounded-t-[1px]">
      ...
      <div className="p-4 bg-white dark:bg-gray-900/50 rounded-b-[1px]">
        ...
        <div className="flex flex-wrap gap-1">
          {data.keywords && data.keywords.length > 0 ? (
            data.keywords.map((kw: string, i: number) => (
              <span key={i} className="px-2 py-1 bg-slate-50 dark:bg-slate-800 text-[9px] font-black text-slate-900 dark:text-white rounded-sm border border-slate-200 dark:border-slate-700 uppercase tracking-tighter">
                {kw}
              </span>
            ))
          ) : (
